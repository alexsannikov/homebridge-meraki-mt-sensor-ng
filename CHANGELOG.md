...
- Config schema is now valid JSON Schema: per-field boolean `required` flags replaced with a `required` array on the device object, and a standard top-level platform `name` property added. Requested by the Homebridge plugin verification pre-checks.
